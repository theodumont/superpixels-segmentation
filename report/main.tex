...
\section{Background on superpixels segmentation}
    ...
    \subsection{A good superpixel segmentation}
        ...
        Let $G = \{G_i\}_i$ and $S = \{S_j\}_j$ be partitions of the same image $I : x_n \mapsto I(x_n)$, $1 \leq n \leq N$. $G$ is the ground truth segmented image\footnote{ground truth segmented or superpixel segmented?\cite{toanswer}} and $S$ is the segmented image obtained from a superpixel algorithm.
        \begin{itemize}
    ...
    \subsection{Algorithms}
        ...
        \subsubsection{Eikonal}
            ...
            Starting from several pixel seeds on the image, we associate a wave to each one. The speed of the wave associated to a pixel is high if it arrives on a pixel that is similar to the original seed, and low otherwise. Wen two wavefronts meet, a contour is generated.
...
\section{Proposed approach and architecture}
    ...
    \subsection{Dataset generation}
        ...
        The COCO dataset contains a huge number of images, but its segmentations are not very qualitative. As it has been labeled by hand in an approximative way, the boundaries of its segmented images are often imprecise (Figure~\ref{fig:imprecise}). We subsequently use the Eikonal algorithm\cite{tochange} to process the images.
